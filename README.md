...
- **播放**：QMediaPlayer 播放器，断点续播 + 播放历史（最近播放视图），Enter/Space 快捷键；键盘控制（←/→ 快退快进 ±5s、↑/↓ 音量、R 倍速循环、F 全屏、Esc 退出/退出全屏）；**播放队列**（⏮⏭ 切换，自然播完自动续播下一集）；**倍速**（0.5x–2.0x 循环，切视频不重置）；**全屏**（F/双击视频区，全屏时 Esc 先退全屏）；**音量记忆**（settings.json）；**断点标记**（时长列 ⏵ 前缀标识可续播，悬停显示续播位置）；**清空播放历史**（工具栏入口，二次确认，断点续播位置一并清除）
...
```bat
python -m pytest tests -q     :: 运行测试（99 个用例）
```
